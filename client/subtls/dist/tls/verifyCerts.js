import { Cert, TrustedCert } from './cert.js';
import { hexFromU8 } from '../util/hex.js';
import { LogColours } from '../presentation/appearance.js';
import { highlightColonList } from '../presentation/highlights.js';
...
export async function verifyCerts(host, certs, rootCertsDatabase, requireServerTlsExtKeyUsage = true, requireDigitalSigKeyUsage = true) {
    // end-user certificate checks
    chatty && log('%c%s', `color: ${LogColours.header}`, 'certificates received from host');
    for (const cert of certs)
        chatty && log(...highlightColonList(cert.description()));
    ...
    for (let i = 0, len = certs.length; i < len; i++) {
        ...
        chatty && log(`verifying certificate CN "${subjectCert.subject.CN}" is signed by %c${signingCertIsTrustedRoot ? 'trusted root' : 'intermediate'}%c certificate CN "${signingCert.subject.CN}" ...`, `background: ${signingCertIsTrustedRoot ? '#ffc' : '#eee'}`, 'background: inherit');
