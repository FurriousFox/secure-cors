...
export async function hkdfExtract(salt, keyMaterial, hashBits) {
    /*
...
export async function hkdfExpand(key, info, length, hashBits) {
    /*
    from https://www.ietf.org/rfc/rfc5869.txt:
...
export async function hkdfExpandLabel(key, label, context, length, hashBits) {
    /*
    ...
    */
    ...
    const hkdfLabel = concat([(length & 0xff00) >> 8, length & 0xff], [tls13_Bytes.length + labelData.length], tls13_Bytes, labelData, [context.length], context);
